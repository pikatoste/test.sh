...
! CURRENT_TEST_NAME= subshell "run_tests test_fail" 3>&1 || false
[ -f "$TEST_SCRIPT_DIR"/.test_fail ]

start_test "When FAIL_FAST is true the first test failure should interrupt the script"
rm -f "$TEST_SCRIPT_DIR"/.test_ok "$TEST_SCRIPT_DIR"/.test_fail
! CURRENT_TEST_NAME= subshell "run_tests test_fail test_ok" 3>&1 || false
...
rm -f "$TEST_SCRIPT_DIR"/.test_ok "$TEST_SCRIPT_DIR"/.test_fail
! FAIL_FAST= SUBSHELL=always CURRENT_TEST_NAME= subshell "run_tests test_fail test_ok" 3>&1 || false
