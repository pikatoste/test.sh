...
( CURRENT_TEST_NAME= result_of "run_tests \"test_fail\"" 3>&1; [[ $LAST_RESULT != 0 ]] )
[ -f "$TEST_SCRIPT_DIR"/.test_fail ]

start_test "When FAIL_FAST is true the first test failure should interrupt the script"
rm -f "$TEST_SCRIPT_DIR"/.test_ok "$TEST_SCRIPT_DIR"/.test_fail
( FAIL_FAST=1 CURRENT_TEST_NAME= result_of "run_tests \"test_fail\" \"test_ok\"" 3>&1; [[ $LAST_RESULT != 0 ]] )
#! CURRENT_TEST_NAME= run_tests test_fail test_ok 3>&1 || false
...
rm -f "$TEST_SCRIPT_DIR"/.test_ok "$TEST_SCRIPT_DIR"/.test_fail
( FAIL_FAST= CURRENT_TEST_NAME= result_of "run_tests \"test_fail\" \"test_ok\"" 3>&1; [[ $LAST_RESULT != 0 ]] )
