...
( CURRENT_TEST_NAME= try_catch_print "run_tests \"test_fail\"" 3>&1; [[ $TRY_EXIT_CODE != 0 ]] )
...
( FAIL_FAST=1 CURRENT_TEST_NAME= try_catch_print "run_tests \"test_fail\" \"test_ok\"" 3>&1; [[ $TRY_EXIT_CODE != 0 ]] )
[ ! -f "$TEST_TMP"/.test_ok ]
[   -f "$TEST_TMP"/.test_fail ]

start_test "When FAIL_FAST is false failures should not interrupt the script but signal failure at the end"
rm -f "$TEST_TMP"/.test_ok "$TEST_TMP"/.test_fail
( FAIL_FAST= CURRENT_TEST_NAME= try_catch_print "run_tests \"test_fail\" \"test_ok\"" 3>&1; [[ $TRY_EXIT_CODE != 0 ]] )
[ -f "$TEST_TMP"/.test_ok ]
[ -f "$TEST_TMP"/.test_fail ]
